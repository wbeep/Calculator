...
void main()
{
	long long first_Num; //Your First Number
	long long second_Num; //Your Second Number
	long long after_Num; //Your Other Number to apply to the previous result with a given operator ex: 2+2 = 4 + 10 <- this is the after_num
	...
	long long check_Stop = 10; //check_Stop is used to stop the while loop if the user declines the option to add, subtract, multiply, divide, or modulus the previous result by a subsequently given number.
	long long past_Output; //previous output
	long long present_Output; //current output
	...
	while (didOneOperation != true)
	{
		...
		second_Num_Prompt_exponent(second_Num, first_Num, past_Output, didOneOperation, operation_Setting);
    }
